...
// console.log "shouldSkip: sfwOnly #{JSON.stringify(sfwOnly)}"
  ...
  calcEntryStrings = function(e) {
    var actualDuration, count, endTime, feeling, nsfwString, opinionString, params, ref, ref1, startTime, title, url, user, userFeeling, whoList;
  ...
  run = async function(args, user) {
    var anonCount, cmd, count, e, extraSkips, i, len, m, name, name1, nameString, newValue, nsfw, oldValue, other, property, q, ref, ref1, ret, strs, title;
    cmd = 'who';
    if (args.length > 0) {
      cmd = args[0];
      cmd = cmd.replace(/^ +/, "");
    ...
    switch (cmd) {
      ...
      case 'skip':
        ...
        for (i = q = 0, ref1 = extraSkips; (0 <= ref1 ? q < ref1 : q > ref1); i = 0 <= ref1 ? ++q : --q) {
          queue.shift();
        }
        e = playNext();
        if (ret == null) {
  ...
  main = async function(argv) {
    ...
    io.on('connection', function(socket) {
      ...
      socket.on('ready', function(msg) {
        var endTime, needsRefresh, sfw, startTime, username;
