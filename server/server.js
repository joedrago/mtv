...
// console.log "shouldSkip: sfwOnly #{JSON.stringify(sfwOnly)}"
  ...
  calcEntryStrings = function(e) {
    var actualDuration, artist, count, endTime, feeling, nick, opinionString, opinionTable, ownerNick, params, ref, ref1, startTime, tag, tagsString, title, url, user, userFeeling, whoList;
  ...
  run = async function(args, user) {
    var anonCount, cmd, companyArgs, concatenatedArgs, count, e, editArgs, extraSkips, filterFunc, i, i1, ignoreArgs, ignoreCmd, ignoreName, index, j, j1, k, legalTags, len, len1, len2, m, name, name1, nameString, newCompany, newNickname, newValue, nicknameArgs, oldValue, other, playArgs, playQueue, playSubstring, playlistCount, playlistID, prettyList, property, q, r, ref, ref1, ref2, ref3, ref4, ref5, ref6, ret, strs, subcommand, tagName, title, unsortedQueue, v, w, x, y, z;
    cmd = 'who';
    if (args.length > 0) {
      cmd = args[0];
      cmd = cmd.replace(/^ +/, "");
    ...
    switch (cmd) {
      ...
      case 'where':
        ...
        strs = calcEntryStrings(lastPlayed);
        return `MTV: ${strs.url}`;
      case constants.opinions[cmd] != null:
      case 'none':
        e = null;
        if (args.length > 1) {
