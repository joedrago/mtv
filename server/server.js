...
(function() {
  var Bottleneck, YOUTUBE_USER, autoPlayNext, autoskip, bodyParser, calcEntryStrings, calcOther, calcUserInfo, checkAutoskip, dashboardsRefreshNeeded, discordEnabled, discordIndex, entryFromArg, express, findMissingYoutubeInfo, fs, getColonTime, getLetterTime, getTime, getYoutubeData, history, https, ident, isAnyoneCasting, isCasting, isPlaying, iso8601, lastPlayed, lastPlayedDuration, lastPlayedTime, lastPlayedTimeout, limiter, load, logOutput, main, now, opinions, output, parseDuration, play, playNext, playingName, playlist, prettyDuration, queue, queueYoutubeTrending, refreshDashboards, refreshDashboardsIfNeeded, requestDashboardRefresh, run, sanitizeUsername, saveOpinions, savePlaylist, saveState, secrets, serverEpoch, sockets, updateCasts, updateDiscord, updateOpinion, updateOpinions, ytdl;
  ...
  autoskip = function() {
    var feeling, ref, shouldSkip, sid, soc, strs;
    if (lastPlayed === null) {
    ...
    for (sid in sockets) {
      ...
      if (ident[sid] != null) {
        feeling = (ref = opinions[lastPlayed.id]) != null ? ref[ident[sid]] : void 0;
        if (feeling == null) {
    ...
    return new Promise(function(resolve, reject) {
      ...
      req = https.request(url, function(res) {
        ...
        return res.on('end', function() {
          var chosenThumb, data, e, i, index, item, j, l, len, len1, len2, m, n, ref, ref1, saved, thumb, thumbType, thumbUrl, trendingQueue, unshuffledTrendingQueue;
          data = null;
          try {
          ...
          if ((data.items != null) && (data.items.length > 0)) {
            ...
            for (index = m = 0, len1 = unshuffledTrendingQueue.length; m < len1; index = ++m) {
              i = unshuffledTrendingQueue[index];
              j = Math.floor(Math.random() * (index + 1));
              trendingQueue.push(trendingQueue[j]);
              trendingQueue[j] = i;
            }
            for (n = 0, len2 = trendingQueue.length; n < len2; n++) {
              e = trendingQueue[n];
              queue.unshift(e);
            }
          }
  ...
  run = async function(args, user) {
    var anonCount, cmd, count, e, extraSkips, i, l, len, m, name, name1, nameString, newValue, oldValue, other, property, ref, ref1, ret, sid, soc, strs, title;
    cmd = 'who';
    if (args.length > 0) {
      cmd = args[0];
      cmd = cmd.replace(/^ +/, "");
    ...
    switch (cmd) {
      ...
      case 'iam':
        for (sid in sockets) {
          soc = sockets[sid];
          if ((playingName[sid] != null) && playingName[sid] === args[1]) {
            ident[sid] = user;
            checkAutoskip();
      ...
      case 'skip':
        ...
        for (i = m = 0, ref1 = extraSkips; (0 <= ref1 ? m < ref1 : m > ref1); i = 0 <= ref1 ? ++m : --m) {
          queue.shift();
        }
        e = playNext();
        if (ret == null) {
  ...
  findMissingYoutubeInfo = function() {
    var k, l, len, missingTitleCount, v;
