...
// console.log "shouldSkip: sfwOnly #{JSON.stringify(sfwOnly)}"
  ...
  run = async function(args, user) {
    var anonCount, cmd, companyArgs, concatenatedArgs, count, e, editArgs, extraSkips, i, len, m, name, name1, nameString, newCompany, newValue, nsfw, oldValue, other, property, q, r, ref, ref1, ref2, ref3, ret, strs, title, u;
    cmd = 'who';
    if (args.length > 0) {
      cmd = args[0];
      cmd = cmd.replace(/^ +/, "");
    ...
    switch (cmd) {
      ...
      case 'skip':
        ...
        for (i = u = 0, ref3 = extraSkips; (0 <= ref3 ? u < ref3 : u > ref3); i = 0 <= ref3 ? ++u : --u) {
          queue.shift();
        }
        e = playNext();
        if (ret == null) {
