...
(function() {
  var AUTOSKIPLIST_COUNT, Bottleneck, YOUTUBE_USER, autoPlayNext, autoskip, autoskipCount, autoskipList, autoskipTimeout, bodyParser, calcEntryStrings, calcOther, calcUserInfo, checkAutoskip, checkIfEveryoneLeft, dashboardsRefreshNeeded, discordEnabled, discordIndex, echoEnabled, echoNewSong, entryFromArg, express, findMissingYoutubeInfo, fs, getColonTime, getLetterTime, getTime, getYoutubeData, history, https, isAnyoneCasting, isCasting, isPlaying, iso8601, lastPlayed, lastPlayedDuration, lastPlayedTime, lastPlayedTimeout, limiter, load, logAutoskip, logOutput, main, nobodyWatchingTimeout, now, opinions, output, parseDuration, play, playNext, playingName, playlist, prettyDuration, queue, queueYoutubeTrending, refreshDashboards, refreshDashboardsIfNeeded, requestDashboardRefresh, run, sanitizeUsername, saveOpinions, savePlaylist, saveState, secrets, serverEpoch, sfwOnly, shouldSkip, sockets, someoneIsWatching, songEnding, songEndingTimeout, splitArtist, updateCasts, updateDiscord, updateOpinion, updateOpinions, ytdl;
  ...
  calcOther = function() {
    var count, n, names, other, sid, soc;
    names = [];
    count = 0;
    for (sid in sockets) {
      soc = sockets[sid];
  ...
  shouldSkip = function(e) {
    var feeling, ref, sid, skipIt, soc;
...
// console.log "shouldSkip: sfwOnly #{JSON.stringify(sfwOnly)}"
    ...
    for (sid in sockets) {
      ...
      if (playingName[sid] != null) {
        feeling = (ref = opinions[e.id]) != null ? ref[playingName[sid]] : void 0;
        if (feeling == null) {
  ...
  songEnding = function() {
    var results, socket, socketId, strs;
    if (lastPlayed == null) {
      return;
    }
    strs = calcEntryStrings(lastPlayed);
    ...
    for (socketId in sockets) {
      ...
      results.push(socket.emit('ending', {
  ...
  play = function(e) {
    var endTime, socket, socketId, startTime, strs;
    lastPlayedTime = now();
    lastPlayed = e;
    history.unshift(e);
  ...
  calcEntryStrings = function(e) {
    var actualDuration, artist, count, endTime, feeling, nsfwString, opinionString, opinionTable, params, ref, ref1, startTime, title, url, user, userFeeling, whoList;
  ...
  run = async function(args, user) {
    var anonCount, cmd, concatenatedArgs, count, e, editArgs, extraSkips, i, len, m, name, name1, nameString, newValue, nsfw, oldValue, other, property, q, r, ref, ref1, ref2, ret, strs, title;
    cmd = 'who';
    if (args.length > 0) {
      cmd = args[0];
      cmd = cmd.replace(/^ +/, "");
    ...
    switch (cmd) {
      ...
      case 'skip':
        ...
        for (i = r = 0, ref2 = extraSkips; (0 <= ref2 ? r < ref2 : r > ref2); i = 0 <= ref2 ? ++r : --r) {
          queue.shift();
        }
        e = playNext();
        if (ret == null) {
  ...
  main = async function(argv) {
    ...
    io.on('connection', function(socket) {
      ...
      socket.on('ready', function(msg) {
        var endTime, needsRefresh, sfw, startTime, strs, username;
