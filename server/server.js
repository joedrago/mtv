...
// console.log "shouldSkip: sfwOnly #{JSON.stringify(sfwOnly)}"
  ...
  run = async function(args, user) {
    var anonCount, block, blockType, cmd, companyArgs, concatenatedArgs, count, e, editArgs, extraSkips, i, i1, ignoreArgs, ignoreCmd, ignoreName, index, j, j1, k, k1, knownTags, l1, legalTags, len, len1, len2, len3, len4, len5, m, m1, n1, name, name1, nameString, newCompany, newNickname, newValue, nicknameArgs, o1, oldValue, other, outputStr, p1, playArgs, playQueue, playSubstring, playlistCount, playlistID, prettyList, property, q, r, ref, ref1, ref10, ref2, ref3, ref4, ref5, ref6, ref7, ref8, ref9, repeatCount, ret, saved, strs, tag, tagArgs, tagName, title, unsortedQueue, v, w, x, y, z;
    cmd = 'who';
    if (args.length > 0) {
      cmd = args[0];
      cmd = cmd.replace(/^ +/, "");
    ...
    switch (cmd) {
      ...
      case 'count':
        ...
        playArgs = [];
        for (i = z = 2, ref4 = args.length; (2 <= ref4 ? z < ref4 : z > ref4); i = 2 <= ref4 ? ++z : --z) {
      ...
      case 'block':
        ...
        blockType = args[1].toLowerCase();
        playArgs = [];
        for (i = j1 = 2, ref6 = args.length; (2 <= ref6 ? j1 < ref6 : j1 > ref6); i = 2 <= ref6 ? ++j1 : --j1) {
          playArgs.push(args[i]);
        }
        playSubstring = playArgs.join(" ").toLowerCase();
        block = generateBlock(blockType, playSubstring);
        ...
        for (index = k1 = 0, len4 = unsortedQueue.length; k1 < len4; index = ++k1) {
          i = unsortedQueue[index];
          j = Math.floor(Math.random() * (index + 1));
          playQueue.push(playQueue[j]);
          playQueue[j] = i;
        }
        for (l1 = 0, len5 = playQueue.length; l1 < len5; l1++) {
          v = playQueue[l1];
          queue.unshift(v);
        }
        saveState();
      ...
      case 'label':
        companyArgs = [];
        for (i = m1 = 1, ref7 = args.length; (1 <= ref7 ? m1 < ref7 : m1 > ref7); i = 1 <= ref7 ? ++m1 : --m1) {
      ...
      case 'name':
        nicknameArgs = [];
        for (i = n1 = 1, ref8 = args.length; (1 <= ref8 ? n1 < ref8 : n1 > ref8); i = 1 <= ref8 ? ++n1 : --n1) {
      ...
      case 'repeat':
        ...
        for (i = o1 = 0, ref9 = repeatCount; (0 <= ref9 ? o1 < ref9 : o1 > ref9); i = 0 <= ref9 ? ++o1 : --o1) {
      ...
      case 'skip':
        ...
        for (i = p1 = 0, ref10 = extraSkips; (0 <= ref10 ? p1 < ref10 : p1 > ref10); i = 0 <= ref10 ? ++p1 : --p1) {
          queue.shift();
        }
        e = playNext();
        if (ret == null) {
